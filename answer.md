...
SELECT *
FROM 
( SELECT PARTY, COUNT(*) AS COUNT_PARTY
FROM PRESIDENT 
WHERE BIRTH_YR > 1850
GROUP BY PARTY ) AS MAIN
WHERE MAIN.COUNT_PARTY =
( SELECT MAX(COUNT_PARTY)
FROM ( SELECT PARTY, COUNT(*) AS COUNT_PARTY
FROM PRESIDENT 
WHERE BIRTH_YR > 1850
GROUP BY PARTY ) AS SUMMARY_PARTY )
```
...
```sql
SELECT PRESIDENT.PARTY, COUNT(*)
FROM PRESIDENT 
WHERE PRESIDENT.BIRTH_YR > 1850
GROUP BY PRESIDENT.PARTY
ORDER BY PRESIDENT.PARTY
```
...
FROM PRES_MARRIAGE 
WHERE PRES_MARRIAGE.NR_CHILDREN >= 2
GROUP BY PRES_MARRIAGE.PRES_NAME
HAVING COUNT(*) >= 2
```
...
ON PRESIDENT.PRES_NAME = ADMINISTRATION.PRES_NAME
WHERE ADMINISTRATION.ADMIN_NR =
( SELECT MIN(ADMINISTRATION.ADMIN_NR)
FROM ADMINISTRATION
WHERE ADMINISTRATION.YEAR_INAUGURATED >= (SELECT MIN(MAR_YEAR) 
FROM PRES_MARRIAGE 
WHERE PRES_MARRIAGE.PRES_NAME = 'Reagan R' ))
```

## 73. แสดงชื่อประธานาธิบดีที่มีอายุน้อยที่สุดเมื่อเข้ารับตําแหน่งครั้งแรก

```sql
SELECT PRESIDENT_FIRST_INAUGURATED.PRES_NAME
FROM 
( SELECT PRESIDENT.PRES_NAME, MIN(ADMINISTRATION.YEAR_INAUGURATED) - MIN(PRESIDENT.BIRTH_YR) AS AGE_AT_FIRST
FROM PRESIDENT
INNER JOIN ADMINISTRATION
ON PRESIDENT.PRES_NAME = ADMINISTRATION.PRES_NAME
GROUP BY PRESIDENT.PRES_NAME
ORDER BY AGE_AT_FIRST ) AS PRESIDENT_FIRST_INAUGURATED
WHERE PRESIDENT_FIRST_INAUGURATED.AGE_AT_FIRST = 
(

SELECT MIN(AGE_AT_FIRST)
FROM
( SELECT PRESIDENT.PRES_NAME, MIN(ADMINISTRATION.YEAR_INAUGURATED) - MIN(PRESIDENT.BIRTH_YR) AS AGE_AT_FIRST
FROM PRESIDENT
INNER JOIN ADMINISTRATION
ON PRESIDENT.PRES_NAME = ADMINISTRATION.PRES_NAME
GROUP BY PRESIDENT.PRES_NAME) AS TEMP
)
```
...
```sql
SELECT  PRES_MARRIAGE.PRES_NAME, SUM(NR_CHILDREN)
FROM PRES_MARRIAGE 
GROUP BY PRES_MARRIAGE.PRES_NAME
HAVING MIN(PR_AGE) < 40 AND COUNT(*) > 1 AND SUM(NR_CHILDREN) > 2
```

## 75. แสดงจํานวนบุตรของประธานาธิบดีที่แต่งงานหลังอายุ 50 ปีและมีคู่สมรสอายุน้อยกว่า 30 ปี

```sql
SELECT PRES_MARRIAGE.PRES_NAME, SUM(PRES_MARRIAGE.NR_CHILDREN)
FROM PRES_MARRIAGE
WHERE PRES_MARRIAGE.PRES_NAME IN 
( SELECT PRES_MARRIAGE.PRES_NAME 
FROM PRES_MARRIAGE 
WHERE PRES_MARRIAGE.PR_AGE > 50 AND PRES_MARRIAGE.SP_AGE < 30 )
GROUP BY PRES_MARRIAGE.PRES_NAME
```
...
```sql
SELECT ELECTION.ELECTION_YEAR, ELECTION.WINNER_LOSER_INDIC, SUM(ELECTION.VOTES)
FROM ELECTION 
GROUP BY ELECTION.ELECTION_YEAR, ELECTION.WINNER_LOSER_INDIC
```

## 77. แสดงอายุเฉลี่ยของประธานาธิบดีที่แต่งงานครั้งแรก

```sql
SELECT AVG(FIRST_AGE)
FROM ( 
  	SELECT PRES_MARRIAGE.PRES_NAME, MIN(PR_AGE) AS FIRST_AGE
	FROM PRES_MARRIAGE 
	GROUP BY PRES_MARRIAGE.PRES_NAME
) AS MAIN
```
...
```sql
SELECT AVG(SECOUND_AGE)
FROM ( 
  	SELECT PRES_MARRIAGE.PRES_NAME, MAX(PR_AGE) AS SECOUND_AGE
	FROM PRES_MARRIAGE 
	GROUP BY PRES_MARRIAGE.PRES_NAME
  	HAVING COUNT(*) = 2
) AS MAIN
...
```sql
SELECT T1.PRES_NAME, T1.BIRTH_YR, T2.PR_AGE
FROM PRESIDENT AS T1
INNER JOIN PRES_MARRIAGE AS T2
ON T1.PRES_NAME = T2.PRES_NAME
WHERE 1 = (
	SELECT COUNT(*)
  FROM PRES_MARRIAGE AS T3
	WHERE T3.PRES_NAME = T1.PRES_NAME AND T2.MAR_YEAR > T3.MAR_YEAR
)
```
...
```sql
SELECT T1.PRES_NAME, T1.BIRTH_YR, T2.ELECTION_YEAR
FROM PRESIDENT AS T1
INNER JOIN ELECTION AS T2
ON T1.PRES_NAME = T2.CANDIDATE
WHERE T2.WINNER_LOSER_INDIC = 'W' AND 
1 = (
	SELECT COUNT(*)
	FROM ELECTION AS T3
  	WHERE T3.CANDIDATE = T2.CANDIDATE AND T2.ELECTION_YEAR > T3.ELECTION_YEAR
)
```
...
WHERE (
	SELECT COUNT(*)
	FROM ELECTION AS T3
  	WHERE T3.CANDIDATE = T2.CANDIDATE AND  T3.WINNER_LOSER_INDIC = 'W'
) = 
 (
	SELECT COUNT(*)
	FROM ELECTION AS T4
  	WHERE T4.CANDIDATE = T2.CANDIDATE AND  T4.WINNER_LOSER_INDIC = 'L'
)
```
